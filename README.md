...
If you turn on `MCP_OAUTH_ENABLED=true` (Keycloak), add the connector with **Advanced settings → OAuth Client ID / OAuth Client Secret** filled in with a Keycloak client's credentials, rather than leaving them blank. Claude Desktop can technically discover the server's OAuth metadata and attempt Dynamic Client Registration on its own, but Keycloak's default "Trusted Hosts" Client Registration Policy rejects anonymous DCR from hosts it doesn't already know about — in practice you need a pre-registered client either way, same as the Claude Code setup below (including registering Claude's callback, `https://claude.ai/api/mcp/auth_callback`, as a **Valid redirect URI** on that client). Once added with credentials, the first connection walks you through a normal browser login/consent flow — there's no static token to paste into the config. See `docs/keycloak-reference-compose.yml` for a throwaway Keycloak realm if you want to test this locally. Clients that can't run a browser-based OAuth flow can instead send an already-issued Keycloak access token directly as an `Authorization: Bearer <token>` header.
