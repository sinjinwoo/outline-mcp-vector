...
**Two protected-resource-metadata paths, on purpose**: when `MCP_OAUTH_ENABLED`, the SDK serves RFC 9728 metadata at whatever path its installed version computes (this has changed across `mcp` SDK versions — seen both bare `/.well-known/oauth-protected-resource` and `/.well-known/oauth-protected-resource/mcp`). Claude Code's MCP client only ever probes `{streamable_http_path}/.well-known/oauth-protected-resource` (i.e. `/mcp/.well-known/oauth-protected-resource`) and doesn't fall back to whichever one the SDK actually serves, so `mcpserver/main.py` registers that exact path too via `@mcp.custom_route(...)`, duplicating the same JSON. Claude Desktop doesn't need this — it's Claude Code specifically. If you ever see OAuth discovery fail for one client but not another, assume the two aren't hitting the same well-known path and check the server's access log for what was actually requested. See `docs/troubleshooting/claude-code-mcp-oauth-compat.md`.
